...
public class ApplicationForm extends JPanel {
    ...
    private void showDataTable() {
        ...
        for (Application transcript : list) {
            model.addRow(new Object[]{
                    ...
                    transcript.getAttendance().getMiddleMark(), transcript.getAttendance().getFinalMark(), transcript.getAttendance().getOtherMark(), transcript.getAttendance().getMark(),
        ...
        applicationTable.getColumnModel().getColumn(12).setCellRenderer(rightRenderer);
        applicationTable.getColumnModel().getColumn(13).setCellRenderer(rightRenderer);
    ...
    private void clearAll() {
        if (clickListener.deleteClick()) {
            String rs = "Có lỗi xảy ra!";
            boolean response = controller.deleteAll(user.getUsername());
            if (response == true) {
                list.removeAll(list);
                model.setRowCount(0);
